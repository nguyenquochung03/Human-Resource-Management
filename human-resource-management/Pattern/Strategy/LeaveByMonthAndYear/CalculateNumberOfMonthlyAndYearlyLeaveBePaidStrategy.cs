...
{
    public class CalculateNumberOfMonthlyAndYearlyLeaveBePaidStrategy : ICalculateNumberOfLeaveByMonthAndYearStrategy
    {
        private readonly HumanResourceManagementDbContext _dbContext;

        public CalculateNumberOfMonthlyAndYearlyLeaveBePaidStrategy(HumanResourceManagementDbContext context)
        {
            _dbContext = context;
        }
        ...
        {
            List<NghiVang> paidLeaveByMonthAndYearList = _dbContext.NghiVangs
                .Where(n => n.NhanVienId == employeeId && n.LoaiNghiVang == LoaiNghiVang.CoLuong
                    && ((n.NgayBatDau.Month == thang && n.NgayBatDau.Year == nam) ||
                    (n.NgayKetThuc.Month == thang && n.NgayKetThuc.Year == nam)))
                .ToList();

            return paidLeaveByMonthAndYearList;
        }
