...
export default function EditWorkScheduleModal({ isOpen, handleCancel, handleConfirm, selectedWorkSchedule, onWorkScheduleUpdated }:EditWorkScheduleModal) {


    const onSubmit = async (values: WorkSchedule, actions: FormikHelpers<WorkSchedule>) => {

        const response = await workScheduleService.edit(values.maLichLam, values);
        if (response?.statusCode === 204) {
            onWorkScheduleUpdated();
            handleCancel();
            actions.resetForm();
